...
#if defined(__GNUC__)
	//return standard values for NaN and Infinities (should be implemented in Single.cs...) (NG 2013)
	if (__builtin_isnan(f))
		return hal_snprintf( buffer, len, "NaN");
	if (__builtin_isinf(f)) 
	{
		if (__builtin_isinf_sign(f) < 0)
			return hal_snprintf( buffer, len, "-Infinity");
		else
			return hal_snprintf( buffer, len, "Infinity");
	}
	INT64   i = (INT64)f;
