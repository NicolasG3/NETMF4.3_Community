...
#if defined(TINYCLR_JITTER)
	{
		BlockStorageDevice* pDevice =  BlockStorageList::GetFirstDevice();
		while (pDevice) {
			ByteAddress address;
			UINT32 blockRegionIndex;
			UINT32 blockRangeIndex;
			if (pDevice->FindForBlockUsage(BlockUsage::JITTER, address, blockRegionIndex, blockRangeIndex)) {
				BlockRegionInfo region = pDevice->GetDeviceInfo()->Regions[blockRegionIndex];
				BlockRange range = region.BlockRanges[blockRangeIndex];
				UINT32 size = range.GetBlockCount() * region.BytesPerBlock;
				FLASH_WORD* start = (FLASH_WORD*) address;
				FLASH_WORD* end   = (FLASH_WORD*) (address + size) ;
				if (!pDevice->IsBlockErased(address, size)) {
					pDevice->EraseBlock(address);
				}
				m_jitter_BlockStorageDevice = pDevice;
				m_jitter_current     = start;
				m_jitter_end         = end;
				break;
			}
			pDevice =  BlockStorageList::GetNextDevice(*pDevice);
		}
		//int                 numSectors;
